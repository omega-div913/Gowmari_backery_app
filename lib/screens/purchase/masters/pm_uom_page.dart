  ...
  Widget _buildHeader() {
    return Row(
      ...
      children: [
        ...
        ElevatedButton.icon(
          onPressed: () => _showUnitDialog(), icon: const Icon(Icons.add, size: 18, color: Colors.white),
  ...
  Widget _buildTableContainer() {
    return Container(
      ...
      child: Column(
        children: [
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15), decoration: BoxDecoration(color: Colors.grey.shade50, border: Border(bottom: BorderSide(color: Colors.grey.shade300))),
            child: const Row(children: [SizedBox(width: 50, child: Text("S.No", style: TextStyle(fontWeight: FontWeight.bold))), Expanded(child: Text("Unit Name", style: TextStyle(fontWeight: FontWeight.bold))), Text("Actions", style: TextStyle(fontWeight: FontWeight.bold))]),
          ),
          ListView.separated(
            shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: paginatedUnits.length, separatorBuilder: (c, i) => const Divider(height: 1),
            itemBuilder: (context, index) {
              ...
              return Padding(
                ...
                child: Row(
                  children: [
                    SizedBox(width: 50, child: Text("$sNo", style: const TextStyle(fontWeight: FontWeight.bold))),
                    Expanded(child: Text(currentUnit)),
                    Row(mainAxisSize: MainAxisSize.min, children: [_actionIcon(Icons.edit_outlined, Colors.blue, () => _showUnitDialog(currentUnit: currentUnit)), const SizedBox(width: 8), _actionIcon(Icons.delete_outline, Colors.red, _showDeleteDialog)]),
                  ],
                ),
              );
            },
