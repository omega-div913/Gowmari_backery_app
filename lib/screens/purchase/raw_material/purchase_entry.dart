...
class _RawMaterialPurchaseEntryPageState extends State<RawMaterialPurchaseEntryPage> {
  ...
  String _vendorSearch = "", _materialSearch = "", _invoiceSearch = "", _fromDate = "", _toDate = "";

  final List<Map<String, dynamic>> _purchaseData = List.generate(25, (index) => {
  ...
  List<Map<String, dynamic>> get filteredData {
    return _purchaseData.where((item) {
      ...
      bool iMatch = item["invoiceNo"].toLowerCase().contains(_invoiceSearch.toLowerCase());
      bool fDateMatch = _fromDate.isEmpty || item["date"].contains(_fromDate);
      bool tDateMatch = _toDate.isEmpty || item["date"].contains(_toDate);
      return vMatch && mMatch && iMatch && fDateMatch && tDateMatch;
    }).toList();
  }
  ...
  void _showDeleteDialog() {
    showDialog(context: context, builder: (ctx) => Dialog(
      ...
      child: Container(width: 450, padding: const EdgeInsets.all(30), child: Column(mainAxisSize: MainAxisSize.min, children: [
  ...
  void _showReverseDialog(Map<String, dynamic> data) {
    showDialog(context: context, builder: (ctx) => Dialog(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
      child: Container(width: 700, child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Reverse Purchase: ${data['invoiceNo']}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), InkWell(onTap: () => Navigator.pop(ctx), child: const Icon(Icons.close, size: 18, color: Colors.grey))])),
        Flexible(child: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [const Text("Vendor: ", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), Text("${data['vendor']}", style: const TextStyle(fontSize: 13))]), const SizedBox(height: 8),
          Row(children: [const Text("Date: ", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), Text("${data['date']}", style: const TextStyle(fontSize: 13))]), const SizedBox(height: 15),
          _buildFormInput("Reversal Reason", "Select Reason", isFilled: false, isDropdown: true), const SizedBox(height: 15),
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text("Additional Notes", style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.black87)), const SizedBox(height: 6), Container(height: 60, padding: const EdgeInsets.all(10), decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(4)), child: const Align(alignment: Alignment.topLeft, child: Text("Explain the reason for reversal...", style: TextStyle(color: Colors.grey, fontSize: 13))))]),
          const SizedBox(height: 20), Row(children: [Container(width: 3, height: 15, color: Colors.red), const SizedBox(width: 8), const Text("Select Items to Reverse", style: TextStyle(fontSize: 13, color: Colors.black87))]), const SizedBox(height: 10),
          Container(decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(4)), child: Column(children: [
            Container(padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 10), decoration: BoxDecoration(color: const Color(0xFFF8F9FA), border: Border(bottom: BorderSide(color: Colors.grey.shade300))), child: Row(children: [_th("MATERIAL", flex: 3), _th("PURCHASED", flex: 2, align: TextAlign.right), _th("PRICE", flex: 2, align: TextAlign.right), _th("REVERSE QTY", flex: 2, align: TextAlign.center), _th("CREDIT AMT", flex: 2, align: TextAlign.right)])),
            Container(padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 10), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade300))), child: Row(children: [const Expanded(flex: 3, child: Text("Kattai Pai", style: TextStyle(fontSize: 13))), const Expanded(flex: 2, child: Text("2772.00", textAlign: TextAlign.right, style: TextStyle(fontSize: 13))), const Expanded(flex: 2, child: Text("₹27.14", textAlign: TextAlign.right, style: TextStyle(fontSize: 13))), Expanded(flex: 2, child: _gridInput("0", isCenter: true)), const Expanded(flex: 2, child: Text("₹0.00", textAlign: TextAlign.right, style: TextStyle(fontSize: 13)))])) ,
            Container(padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 10), decoration: const BoxDecoration(color: Color(0xFFF8F9FA)), child: const Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Total Reversal Amount:", style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)), Text("₹0.00", style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.red))]))
          ])), const SizedBox(height: 20),
          Container(padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: const Color(0xFFF8F9FA), borderRadius: BorderRadius.circular(4), border: Border.all(color: Colors.grey.shade300)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text("Financial Impact Preview:", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)), const SizedBox(height: 15), _summaryRow("Reversal Total:", "₹0.00", fontSize: 13), _summaryRow("Offset Outstanding Balance:", "-₹0.00", color: Colors.green, fontSize: 13), const Divider(), _summaryRow("Net Vendor Credit:", "+₹0.00", color: Colors.blue, fontSize: 13)]))
        ]))),
        Container(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15), decoration: BoxDecoration(border: Border(top: BorderSide(color: Colors.grey.shade200))), child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
  ...
  Widget _buildListView() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey.shade300)), child: Column(children: [
        Row(children: [
          Expanded(child: _buildFilterInput("From Date", "18-05-2026", isDate: true, onChanged: (v) => setState((){ _fromDate = v; _currentPage = 1;}))), const SizedBox(width: 20), 
          Expanded(child: _buildFilterInput("To Date", "18-05-2026", isDate: true, onChanged: (v) => setState((){ _toDate = v; _currentPage = 1;}))), const SizedBox(width: 20), 
          Expanded(child: _buildFilterInput("Search by Vendor", "Type or select a vendor...", onChanged: (v) => setState((){ _vendorSearch = v; _currentPage = 1;})))
        ]), const SizedBox(height: 15),
        Row(children: [
          Expanded(flex: 2, child: _buildFilterInput("Search by Material", "Type or select a material...", onChanged: (v) => setState((){ _materialSearch = v; _currentPage = 1;}))), const SizedBox(width: 20), 
          Expanded(flex: 1, child: _buildFilterInput("Search by Invoice", "Enter invoice no...", onChanged: (v) => setState((){ _invoiceSearch = v; _currentPage = 1;})))
        ])
      ])), const SizedBox(height: 25),
      ...
      Container(width: double.infinity, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey.shade300)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        LayoutBuilder(builder: (context, constraints) {
          return SingleChildScrollView(scrollDirection: Axis.horizontal, child: SizedBox(width: constraints.maxWidth < 1100 ? 1100 : constraints.maxWidth, child: Column(children: [
            Container(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15), decoration: BoxDecoration(color: const Color(0xFFF8F9FA), border: Border(bottom: BorderSide(color: Colors.grey.shade300))), child: const Row(children: [Expanded(flex: 1, child: Text("S.No", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))), Expanded(flex: 2, child: Text("Date", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))), Expanded(flex: 2, child: Text("Invoice No.", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))), Expanded(flex: 3, child: Text("Vendor", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))), Expanded(flex: 4, child: Text("Material(s)", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))), Expanded(flex: 3, child: Text("Batch", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))), Expanded(flex: 2, child: Text("Total", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13), textAlign: TextAlign.right)), Expanded(flex: 2, child: Text("Images", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13), textAlign: TextAlign.center)), Expanded(flex: 2, child: Text("Status", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13), textAlign: TextAlign.center)), Expanded(flex: 4, child: Text("Actions", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13), textAlign: TextAlign.right))])),
            ...paginatedData.asMap().entries.map((entry) {
              ...
              return Container(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))), child: Row(children: [
                Expanded(flex: 1, child: Text("${((_currentPage - 1) * _itemsPerPage) + entry.key + 1}", style: const TextStyle(fontSize: 13))), Expanded(flex: 2, child: Text(data["date"], style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold))), Expanded(flex: 2, child: Text(data["invoiceNo"], style: const TextStyle(fontSize: 13))), Expanded(flex: 3, child: Text(data["vendor"], style: const TextStyle(fontSize: 13))), Expanded(flex: 4, child: Text(data["material"], style: const TextStyle(fontSize: 13))), Expanded(flex: 3, child: Text(data["batch"], style: const TextStyle(fontSize: 13))),
  ...
  Widget _buildNewPurchaseForm() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(_isEditMode ? "Edit Purchase" : "New Purchase Entry", style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w600, color: Colors.black87)),
      ...
      _buildSectionCard(title: "Invoice Details", child: Column(children: [
        Row(children: [
          Expanded(child: _buildFormInput("Vendor", _isEditMode ? "DSP Bags" : "Select a vendor...", isDropdown: true)),
          if (!_isEditMode) ...[const SizedBox(width: 15), Expanded(child: _buildFormInput("Link Purchase Order", "Manual Entry", isDropdown: true))],
          const SizedBox(width: 15), Expanded(child: _buildFormInput("Invoice Number", _isEditMode ? "28" : "Enter or generate...", suffixIcon: Icons.settings, suffixColor: Colors.blue)),
      ...
      _buildSectionCard(title: "Purchase Items", headerTrailing: Row(children: [
        ...
        ElevatedButton.icon(onPressed: () => setState(() => _formItems.add(Map.from(_emptyItemTemplate))), icon: const Icon(Icons.add_circle_outline, size: 16), label: const Text("Add Item (Alt+N)"), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0056B3), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(4)), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12)))
        ...
        ..._formItems.asMap().entries.map((entry) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Expanded(flex: 3, child: _gridInput(item["material"] ?? "Select...", isDropdown: !_isEditMode)), const SizedBox(width: 8), Expanded(flex: 2, child: _gridInput("dd-mm-yyyy", suffixIcon: Icons.calendar_today)), const SizedBox(width: 8), Expanded(flex: 1, child: _gridInput(item["unit"] ?? "N/A", isFilled: true, isCenter: true)), const SizedBox(width: 8),
            Expanded(flex: 1, child: Center(child: Text(item["prevPrice"] ?? "-", style: TextStyle(color: item["prevPrice"] != "-" ? Colors.blue : Colors.grey.shade600, fontSize: 12)))), const SizedBox(width: 8), Expanded(flex: 1, child: _gridInput(item["price"] ?? "0", isCenter: true)), const SizedBox(width: 8), Expanded(flex: 1, child: _gridInput(item["qty"] ?? "0", isCenter: true)), const SizedBox(width: 8),
            Expanded(flex: 1, child: _gridInput(item["subTotal"] ?? "0.00", isCenter: true, isBold: true, borderColor: Colors.grey.shade300)), const SizedBox(width: 8), Expanded(flex: 2, child: _gridInput("SGST + CGST", isDropdown: true)), const SizedBox(width: 8), Expanded(flex: 1, child: Column(children: [_gridInput("0", suffixText: "%"), const SizedBox(height: 4), _gridInput("0", isCenter: true)])), const SizedBox(width: 8), Expanded(flex: 1, child: Column(children: [_gridInput("0", suffixText: "%"), const SizedBox(height: 4), _gridInput("0", isCenter: true)])), const SizedBox(width: 8),
            Expanded(flex: 1, child: _gridInput(item["total"] ?? "₹0.00", isFilled: true, isCenter: true, isBold: true)), const SizedBox(width: 8), InkWell(onTap: () => setState(() => _formItems.removeAt(index)), child: Container(height: 32, width: 32, decoration: BoxDecoration(border: Border.all(color: Colors.red.shade300), borderRadius: BorderRadius.circular(4)), child: const Icon(Icons.delete_outline, color: Colors.red, size: 18)))
          ]));
        }).toList()
      ])), const SizedBox(height: 20),
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Expanded(flex: 6, child: Column(children: [
          _buildSectionCard(title: "Initial Payment", child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Expanded(child: _buildFormInput("Paid Amount", "0", prefixText: "₹")), const SizedBox(width: 15), Expanded(child: _buildFormInput("Payment Method", "Select...", isDropdown: true))]), const SizedBox(height: 15), _buildFormInput("Payment Notes", "Optional: Cheque number, transaction ID, etc.")])), const SizedBox(height: 20),
  ...
  // --- REUSABLE WIDGETS ---
  Widget _buildSectionCard({required String title, required Widget child, Widget? headerTrailing, IconData? titleIcon}) => Container(width: double.infinity, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey.shade300)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 15), decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey.shade200))), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Row(children: [if (titleIcon != null) ...[Icon(titleIcon, size: 18, color: Colors.blue), const SizedBox(width: 8)], Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold))]), if (headerTrailing != null) headerTrailing])), Padding(padding: const EdgeInsets.all(20), child: child)]));
  
  // Replaced static text Container with actual TextField
  Widget _buildFilterInput(String label, String hint, {bool isDate = false, ValueChanged<String>? onChanged}) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.black87)), const SizedBox(height: 8), Container(height: 38, decoration: BoxDecoration(color: Colors.white, border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(4)), child: TextField(onChanged: onChanged, style: const TextStyle(fontSize: 13, color: Colors.black87), decoration: InputDecoration(hintText: hint, hintStyle: const TextStyle(fontSize: 13, color: Colors.grey), contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12), border: InputBorder.none, suffixIcon: isDate ? const Icon(Icons.calendar_today_outlined, size: 16, color: Colors.black54) : null)))]);
  
  Widget _buildFormInput(String label, String hint, {bool isDropdown = false, IconData? suffixIcon, Color? suffixColor, bool isFilled = false, String? prefixText}) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.black87)), const SizedBox(height: 6), Container(height: 38, padding: const EdgeInsets.symmetric(horizontal: 12), decoration: BoxDecoration(color: isFilled ? const Color(0xFFF8F9FA) : Colors.white, border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(4)), child: Row(children: [if (prefixText != null) ...[Text(prefixText, style: const TextStyle(color: Colors.black54, fontSize: 13)), const SizedBox(width: 8)], Expanded(child: Text(hint, style: TextStyle(fontSize: 13, color: isFilled ? Colors.black87 : Colors.grey))), if (isDropdown) const Icon(Icons.keyboard_arrow_down, size: 16, color: Colors.black54), if (suffixIcon != null) Container(padding: const EdgeInsets.all(4), decoration: BoxDecoration(border: Border.all(color: suffixColor!.withOpacity(0.3)), borderRadius: BorderRadius.circular(4)), child: Icon(suffixIcon, size: 12, color: suffixColor))]))]);
  ...
  Widget _gridInput(String hint, {bool isDropdown = false, IconData? suffixIcon, bool isFilled = false, bool isCenter = false, bool isBold = false, String? suffixText, Color? borderColor}) => Container(height: 32, padding: const EdgeInsets.symmetric(horizontal: 8), decoration: BoxDecoration(color: isFilled ? const Color(0xFFF8F9FA) : Colors.white, border: Border.all(color: borderColor ?? Colors.grey.shade300), borderRadius: BorderRadius.circular(4)), child: Row(mainAxisAlignment: isCenter ? MainAxisAlignment.center : MainAxisAlignment.spaceBetween, children: [Expanded(child: Text(hint, textAlign: isCenter ? TextAlign.center : TextAlign.left, style: TextStyle(fontSize: 12, color: isFilled ? Colors.black87 : Colors.grey.shade600, fontWeight: isBold ? FontWeight.bold : FontWeight.normal), overflow: TextOverflow.ellipsis)), if (isDropdown) const Icon(Icons.keyboard_arrow_down, size: 14, color: Colors.black54), if (suffixIcon != null) Icon(suffixIcon, size: 14, color: Colors.black54), if (suffixText != null) Text(suffixText, style: const TextStyle(fontSize: 12, color: Colors.grey))]));
